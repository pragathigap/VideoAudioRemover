...
const ToolPage: React.FC<ToolPageProps> = ({ mode, title, description, primaryKeyword }) => {
  ...
  useEffect(() => {
    ...
    document.title = `${plainTitle} | Remove Audio From Video`;
    ...
    if (primaryKeyword) {
      ...
      if (metaKeywords) {
        ...
        if (!currentKeywords.toLowerCase().includes(primaryKeyword.toLowerCase())) {
          metaKeywords.setAttribute('content', `${primaryKeyword}, ${currentKeywords}`);
        }
      }
    }
